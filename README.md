...
1. Accept a direct Douyin video URL, a `v.douyin.com` short URL, or the complete share message copied from Douyin.
2. Ask the locally installed Douzy sidecar to download the requested video, or reuse its existing cached copy.
...
| Douzy | Yes | `%LOCALAPPDATA%\Programs\Douzy` | Downloads Douyin videos and maintains the local history/database. |
| Douzy configuration | Yes | `%APPDATA%\douyin-downloader-desktop\config.yml` | Provides the download directory and sidecar configuration. |
...
- When VietSub Studio opens its dedicated Edge window, sign in to the Google account that can access this Notebook.
...
Use the Edge window created by VietSub Studio, sign in to Google, confirm the Notebook opens, and make sure the account has access. Do not close the dedicated Edge debug profile while processing.
...
Not in version 1.0.0. The current translation adapter automates Gemini Notebook through Edge. A future API adapter can be proposed through an issue or pull request.
...
No. The current URL validator and download integration intentionally support Douyin only.
...
1. Nhận link video Douyin trực tiếp, link ngắn `v.douyin.com` hoặc nguyên đoạn chia sẻ sao chép từ Douyin.
2. Yêu cầu sidecar Douzy đã cài trên máy tải đúng video hoặc dùng lại bản đã có trong cache.
...
| Douzy | Có | `%LOCALAPPDATA%\Programs\Douzy` | Tải video Douyin và quản lý lịch sử/cache cục bộ. |
| Cấu hình Douzy | Có | `%APPDATA%\douyin-downloader-desktop\config.yml` | Cung cấp thư mục tải và cấu hình sidecar. |
...
- Khi VietSub Studio mở cửa sổ Edge riêng, đăng nhập tài khoản Google có quyền truy cập Notebook đó.
...
Dùng đúng cửa sổ Edge do VietSub Studio mở, đăng nhập Google, xác nhận Notebook truy cập được và không đóng profile Edge debug trong lúc xử lý.
...
Chưa ở phiên bản 1.0.0. Adapter hiện tại tự động hoá Gemini Notebook qua Edge. Có thể đề xuất adapter API bằng Issue hoặc pull request.
...
Không. Validator URL và tích hợp tải hiện chỉ hỗ trợ Douyin.
